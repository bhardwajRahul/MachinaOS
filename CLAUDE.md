...
| **[Schema Source of Truth RFC](./docs-internal/schema_source_of_truth_rfc.md)** | Wave 3 + Wave 6: backend is SSOT for node output schemas AND input parameter schemas via Pydantic → JSON Schema. 110 NodeSpecs registered. Endpoints: `GET /api/schemas/nodes/{type}.json` (output), `/{type}/spec.json` (full NodeSpec), `/nodes/options/{method}` (loadOptions dispatcher), `/nodes/groups` (node-groups index). Frontend adapter + feature flag `VITE_NODESPEC_BACKEND` ready; Phase 3e flag flip + frontend nodeDefinitions/* deletion deferred pending snapshot tests. |
