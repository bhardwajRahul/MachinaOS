...
class ClaudeTaskSpec(BaseAICliTaskSpec):
    ...
    session_id: Optional[str] = Field(
        default=None,
        description="Session UUID to start under (`--session-id`; must be "
        "a valid UUID). Minted by the session pool on cold "
        "spawn when unset; ignored when `resume_session_id` or "
        "`continue_session` is set.",
    )
    resume_session_id: Optional[str] = Field(
        default=None,
        description="Resume a specific prior session UUID (`--resume`). "
        "Mutually exclusive with `continue_session`. Memory-bound "
        "runs set this from the memory node's `last_session_id`.",
    )
    continue_session: bool = Field(
        default=False,
        description="Emit `--continue` (most recent conversation under the "
        "current cwd). Note the CLI skips non-interactive "
        "sessions here, so it does not find sessions this pool "
        "created; prefer `resume_session_id`.",
    )
